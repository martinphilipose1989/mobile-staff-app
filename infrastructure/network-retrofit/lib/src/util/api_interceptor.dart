import 'package:data/data.dart';
import 'package:dio/dio.dart';
import 'package:get_it/get_it.dart';
import 'package:injectable/injectable.dart';
import 'package:network_retrofit/src/util/network_properties.dart';
import 'package:themes/themes.dart';

class ApiInterceptor extends InterceptorsWrapper {
  final String apiKey;
  final SharedPreferencesService sharedPreferencesService =
      GetIt.I<SharedPreferencesService>();

  ApiInterceptor(@factoryParam this.apiKey);

  @override
  Future onRequest(
      RequestOptions options, RequestInterceptorHandler handler) async {
    final isLoginRequest =
        options.uri.path.contains(NetworkProperties.gateLogin);
    final isCoReasonRequest = options.uri.path.contains("/api/co-reasons");

    if (!isLoginRequest) {
      if (isCoReasonRequest) {
        options.headers.putIfAbsent(
            ServerConstants.authorization,
            () =>
                "Bearer ");
      } else {
        options.headers.putIfAbsent(ServerConstants.authorization,
            () => "${sharedPreferencesService.getFromDisk("token")}");
      }
    }

    return handler.next(options);
  }

  @override
  Future onResponse(
      Response response, ResponseInterceptorHandler handler) async {
    handler.next(response);
  }
}
